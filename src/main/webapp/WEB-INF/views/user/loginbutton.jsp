...
<head>
<meta charset="UTF-8">
<title>import login button</title>
<link
...
<div id="loginform" align="right">
			...
			<!-- 관리자로 로그인이 된 상태인 경우 (마이페이지 및 메뉴 항목 수정 예정)//이름 출력 되는 부분을 수정해야함 -->
			<c:if
				test="${ !empty sessionScope.loginUser and loginUser.admin_yn eq 'Y' }">
	${ sessionScope.loginUser.username } 관리자님 어서오세요. &nbsp; &nbsp;
	<button class="btn btn-outline-primary"
					onclick="javascript:location.href='logout.do';">로그아웃</button>  &nbsp; &nbsp; &nbsp; &nbsp;
			</c:if>
			<!-- 일반 회원이 로그인 된 경우 (마이페이지 및 메뉴 항목 수정 예정) // 이름 출력 되는 부분을 수정해야함 -->
			<c:if
				test="${ !empty sessionScope.loginUser and loginUser.admin_yn eq 'N' }">
	${ sessionScope.loginUser.username } 회원님 어서오세요.  &nbsp; &nbsp;
	<button class="btn btn-outline-primary"
					onclick="javascript:location.href='logout.do';">로그아웃</button> &nbsp; &nbsp; &nbsp; &nbsp;
			</c:if>
		</div>
