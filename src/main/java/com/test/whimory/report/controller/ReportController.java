...
public class ReportController {
	...
	public ModelAndView reportDetailUMethod(ModelAndView mv, @RequestParam("report_no") int report_no,
		...
		@RequestMapping("rpdetaila.do")
		public ModelAndView reportDetailAdminMethod(ModelAndView mv, @RequestParam("report_no") int report_no,
				@RequestParam("page") int page) {
			// 조회수 1 증가 처리
			reportService.updateAddReadCount(report_no);
			...
			if (report != null) {
				mv.addObject("report", report);
				mv.addObject("currentPage", page);
				mv.setViewName("user/reportAdminDetailView");
			} else {
				mv.addObject("messeage", report_no + "번 게시글 조회 실패.");
				mv.setViewName("common/error");
			}
